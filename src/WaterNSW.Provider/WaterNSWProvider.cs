...
{
    public class WaterNSWProvider : ProviderBase, IExtendedProviderMetadata
    {
        private readonly IWaterNSWClientFactory _WaterNSWClientFactory;

        public WaterNSWProvider([NotNull] ApplicationContext appContext, IWaterNSWClientFactory WaterNSWClientFactory)
            : base(appContext, WaterNSWConstants.CreateProviderMetadata())
        {
            _WaterNSWClientFactory = WaterNSWClientFactory;
        }
            ...
            Guid providerDefinitionId)
        {
            ...
            var WaterNSWCrawlJobData = new WaterNSWCrawlJobData();
            if (configuration.ContainsKey(WaterNSWConstants.KeyName.ApiKey))
            { WaterNSWCrawlJobData.ApiKey = configuration[WaterNSWConstants.KeyName.ApiKey].ToString(); }

            return await Task.FromResult(WaterNSWCrawlJobData);
        }
            ...
            Guid providerDefinitionId)
        {
            ...
            if (jobData is WaterNSWCrawlJobData WaterNSWCrawlJobData)
            {
                ...
                dictionary.Add(WaterNSWConstants.KeyName.ApiKey, WaterNSWCrawlJobData.ApiKey);
            }
        ...
        public override async Task<AccountInformation> GetAccountInformation(ExecutionContext context, [NotNull] CrawlJobData jobData, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            ...
            var client = _WaterNSWClientFactory.CreateNew(WaterNSWCrawlJobData);
        ...
        public string Icon => WaterNSWConstants.IconResourceName;
        public string Domain { get; } = WaterNSWConstants.Uri;
        public string About { get; } = WaterNSWConstants.CrawlerDescription;
        public AuthMethods AuthMethods { get; } = WaterNSWConstants.AuthMethods;
        public IEnumerable<Control> Properties => null;
        public string ServiceType { get; } = JsonConvert.SerializeObject(WaterNSWConstants.ServiceType);
        public string Aliases { get; } = JsonConvert.SerializeObject(WaterNSWConstants.Aliases);
        public Guide Guide { get; set; } = new Guide
        {
            Instructions = WaterNSWConstants.Instructions,
            Value = new List<string> { WaterNSWConstants.CrawlerDescription },
            Details = WaterNSWConstants.Details

        };

        public string Details { get; set; } = WaterNSWConstants.Details;
        public string Category { get; set; } = WaterNSWConstants.Category;
        public new IntegrationType Type { get; set; } = WaterNSWConstants.Type;
    }
}
