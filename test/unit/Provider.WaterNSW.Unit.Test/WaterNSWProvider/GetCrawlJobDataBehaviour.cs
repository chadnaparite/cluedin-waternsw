...
{
    public class GetCrawlJobDataBehaviour : WaterNSWProviderTest
    {
        ...
        [Theory]
        [InlineAutoData(WaterNSWConstants.KeyName.ApiKey, nameof(WaterNSWCrawlJobData.ApiKey))]
        ...
        [Theory]
        [InlineAutoData]
        public async Task WaterNSWCrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.WaterNSW.Core.WaterNSWCrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
